...
Ивенты (только создатель):
• ивент money [секунды] - x2 к выигрышам
• ивент crash [секунды] - минимальный множитель x1.1 в краше
• ивент статус - текущий ивент
• ивент стоп - завершить досрочно
• ивент история - история ивентов

Админ-команды:
• логи - все логи
• логи [ID] - логи конкретного админа
• топ общий - общий топ (наличные + карта)
• топ общий [ID] - общий баланс конкретного пользователя
• выдать статус [название] (ответом) - выдать статус
• список статусов - все доступные статусы
